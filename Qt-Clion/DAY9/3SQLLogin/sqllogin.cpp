...
void SQLLogin::on_pushButton_reg_clicked()
{
    ...
    if(password==0|password==""){
        QMessageBox::warning(this,"提示","密码为空,注册失败!");
...
//    //2.放入要加密的数据
//    hash1.addData(username.toUtf8());
//    //3.取出加密结果
//    QByteArray arr1 = hash.result();
//    username = arr1.toHex();
//    qDebug()<<username;
    //-----------------------------
    ...
    if(!query.exec(sql)){
        if(!query.exec(QString("insert into user(username,password) values('%1')").arg(username))){
         QMessageBox::warning(this,"提示","用户名重复,注册失败!");
          return;
        }
