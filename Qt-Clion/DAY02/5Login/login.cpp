...
Login::Login(QWidget *parent)
    ...
    //将对应的LineEdit设置为Label的伙伴
    username_lab->setBuddy(username_edit);
    password_lab->setBuddy(password_edit);
    ...
    this->setLayout(lyt);
    //连接回车信号和登录按钮信号
    QObject::connect(password_edit, SIGNAL(returnPressed()), login_btn, SIGNAL(clicked()));
    //退出按钮连接关闭窗口的槽
    ...
    QObject::connect(login_btn, &QPushButton::clicked, this, [&]() {
        if (username_edit->text().isEmpty() || password_edit->text().isEmpty()) {//用户名或密码为空
            ...
            if (QMessageBox::Ok ==
                QMessageBox::warning(this, "提示", "用户名或密码为空", QMessageBox::Ok | QMessageBox::Cancel)) {
                qDebug() << "QMessageBox::Ok!";
            } else {
                qDebug() << "QMessageBox::Cancel!";
            }
            ...
            */
            ...
            qDebug() << "用户名或密码错误";
            if (QMessageBox::Ok ==
                QMessageBox::warning(this, "提示", "用户名或密码错误", QMessageBox::Ok | QMessageBox::Cancel)) {
                qDebug() << "QMessageBox::Ok!";
                username_edit->clear();
                password_edit->clear();//清空edit
            } else {
                qDebug() << "QMessageBox::Cancel!";
            }
        }
