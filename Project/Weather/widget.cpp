    ...
    ui(new Ui::Widget)
{
    ...
    /*获取天气API*/
    QString str = "http://wthrcdn.etouch.cn/weather_mini?city=揭阳";
    QUrl url(str);
...
void Widget::doProcessfinished(QNetworkReply *reply)
{
    ...
    if(fore.isArray())
    {
        ...
        for(int i = 0;i<4;i++)
        {
            ...
            QString Week = weaobj.value("date").toString().mid(2);//星期
...
void Widget::up()
{
    ...
    animation_text_weather_left->setEndValue(QPoint(ui->frame_pic->pos().x() + 70,label_curr_weather->pos().y() - 100));
    ...
    animation_future_up_1->setEndValue(QPoint(ui->frame_future->pos().x(),ui->frame_future->pos().y() - ui->frame_future->height() - 400));
...
void Widget::down()
{
    ...
    animation_future_down->setEndValue(QPoint(ui->frame_future->pos().x(),ui->frame_future->pos().y() + ui->frame_future->height() + 400));
